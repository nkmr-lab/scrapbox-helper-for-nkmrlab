...
### タイマー（ローカル・3ベル方式）
- フロートメニューの「タイマー」ボタンで起動
- 3状態モデル:
  - **idle（設定）**: 1ベル / 2ベル / 3ベル（mm:ss）と「発表終了」ベルを設定、`OK` で armed へ
  - **armed（スタート待機）**: 00:00 表示、`▶ スタート` で計測開始、`設定` で idle に戻る
  - **running / paused**: 計測中。`⏸/▶` 一時停止、`↺` で同じ設定のまま先頭（armed）に戻す、`設定` で idle へ
- デフォルト: 1ベル 4:00 / 2ベル 5:00 / 3ベル 10:00 / 発表終了 = 2ベル
- 各ベル到達時に n回 のビープ音（1ベル=1回、2ベル=2回、3ベル=3回）
- **時刻表示クリックで 経過 ↔ 残り（発表終了ベルまでのカウントダウン）を切替**
- 色変化は 1ベル超=黄 / 発表終了ベル超=赤 / 3ベル超=フラッシュ
- 入力値・表示モード・ビープON/OFFは chrome.storage.local に自動保存、次回起動時に復元
- ページ遷移・リロードを跨いで継続動作
- すべての設定はタイマーウィジェット内で完結（メイン設定モーダルにはタイマータブはない）

### 発言者解決
- 各行の uid を一次キーとする
- 表示名は **その時点で取得した** ページの `json.collaborators` から都度引く（永続キャッシュなし、ページ遷移ごとに再構築）
- collaborators に無い uid は、当該ページのアイコン記法 `[name.icon]` で補完
- どちらも無ければ uid を生のまま表示（同名取り違えや別ページのキャッシュ汚染を避けるため）
